...
| `chats` | `[ChatUsuarioSchema]` | `ultimomensaje_e2ee` (X25519, this user's key), `nombre_bloqueo` (system key) | Per-user view of each chat: pin/mute/block flags, `ultimoCambio`, the last-message preview, and a "freeze" snapshot (`mensaje_bloqueo_id`, `nombre_bloqueo`, `participantes_bloqueo`) taken at expulsion/block time so a removed/blocked user still sees the chat as it was. The preview lives in `ultimomensaje_e2ee`, wrapped with the owning user's public key; the legacy `ultimomensaje` (system key) is still read for existing rows and for the fixed system notices written by `ChatRepository.js`, but user-authored subjects are no longer written to it. Decrypted by `_descifrarUltimoMensaje()` in `UserRepository.js`. |
...
| `contenido` | `[{ asunto: EncryptedDataSchema, archivos: [ArchivoSchema] }]` | No, for new messages | Non-secret index only. New messages write `asunto: null` and keep just `{id, iv, tag}` per attachment (the GridFS pointers). Legacy messages still hold the old system-key copy of the subject and filenames, which is read as a fallback but never written again. Always an array but only ever holds one element in practice. |
...
| `claves_recuperacion` | `[{ usuario_id, clave: {ephPub, iv, data, tag} }]` | Yes (X25519, per participant) | **Key escrow.** The message key wrapped to each participant's X25519 public key, same construction as `ratchet_keys.clave_envuelta`. Lets any participant recover the message (and its attachments) when the ratchet state desynchronizes, without the build-wide system key. Replaces the old `asunto`/`key_enc` system copy — see `Docs/backend/CRYPTO_SECURITY.md` §3.7. |
| `ratchet_info` | `{ iteration, chain_id }` | — | Which ratchet step produced this message's key, needed to re-derive the key on read. |

`ArchivoSchema` (embedded in `contenido[].archivos`): `id` (GridFS file id), `iv`, `tag` (plain strings — the file's own AES-GCM parameters), plus two legacy-only fields, `nombre` and `key_enc` (`EncryptedDataSchema`, system key). New messages populate only `id`/`iv`/`tag`: the filename now travels inside the ratchet-encrypted payload, and the file's key comes from `claves_recuperacion` instead of `key_enc`. Both legacy fields are still read for messages that predate the escrow.
...
| `DESCARGAR_ARCHIVO` / `OBTENER_PREVIEW_IMAGEN` / `OBTENER_AUDIO_MENSAJE` | Stream a file out of GridFS (`GridFSBucket`, bucket `ArchivosChats`), deriving the file's decryption key via `getMessageKey()` (re-derives from the ratchet at the message's recorded iteration). If the ratchet has rotated past that point, `_getFileKeyFromMessage()` unwraps the message's `claves_recuperacion` escrow with the local X25519 private key, falling back to the legacy `key_enc` only for messages that predate the escrow. |
