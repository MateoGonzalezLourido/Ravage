...
| `ajustes.js` | Settings panel: account data changes (password/nickname/email flows — the password form `#form-cambio-contraseña` now includes a `#cambio-pass-actual` field, and its value is forwarded as `contraseña_actual` to `window.cuenta_usuario.PERMITIR_CAMBIO_DATOS_CUENTA({ data, tipo, contraseña_actual })`, which the backend verifies before allowing the change), security-scanner toggles per user, muted/blocked-user lists (`ver_chats_silenciados`, `ver_chats_bloqueados`), visual "threads" toggle (`aplicar_ajuste_hilos`). Largest UI file after `chat.js` (1344 lines). |
...
| `mensajes_eventos.js` | Message composer logic: `@mention` autocomplete dropdown (navigation, selection), typing-indicator handling, sending a message (`enviar_mensaje_chat`), responding to "add to group" requests, per-message context menu. `enviar_mensaje_chat` wraps the `window.chats.ENVIAR_MENSAJE` call in `try/catch` and also checks `result.success`: on either failure path it calls `restaurar_borrador_fallido(...)` — putting the original text, the staged attachments and the pending mentions back into the composer — and surfaces an error toast via `pushNotificacion`, so a failed send no longer discards the message silently. `renderer.js` additionally attaches a `.catch` to the call. Its `[MENCIONES]` debug logs are gated behind `window.opciones_dev?.isDev`. |
...
| `seguridad_ui.js` | Tiny utility module: `escapeHTML()` (XSS-safe HTML escaping for interpolated text; casts its argument with `String()` and also escapes backticks) and `safeIdSelector()` (safely builds a CSS ID selector from arbitrary IDs). This is the **single** definition of `escapeHTML` in the frontend — the divergent local copy that used to live in `chat.js` was removed, and `chat.js`, `historial_archivos_descargados.js` and the rest import it from here. |
...
- **Message rendering** — `crear_mensaje_html` builds the HTML for a single message (text via `marked`, sanitized via `DOMPurify` in `sanitizarTexto`, plus attachments/mentions/date grouping). `ALLOWED_TAGS` deliberately excludes `a`: markdown links are turned into `<span class="url-mensaje" data-url>` elements instead, so the old `DOMPurify.addHook('afterSanitizeAttributes', ...)` that forced `target`/`rel` on anchors never fired and has been deleted (a comment in its place documents what would be needed to re-enable real links). `_construir_html_mensajes`, `_renderizar_bloque_en_dom`, `_rearmar_agrupacion_dom` and `_agrupar_por_dia`/`_calcular_agrupacion` implement day-grouping of consecutive messages.
